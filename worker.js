...
async function rebuildQualityScoresTable(connection) {
  await connection.prepare("DROP TABLE IF EXISTS quality_scores__new").run();
  await connection.prepare(
    "CREATE TABLE quality_scores__new (" +
  ...
  await connection.prepare(
    "INSERT INTO quality_scores__new (" +
      "id, user_id, reference_month, score, score_type, quality_scope, " +
      "monitoria_1, monitoria_2, monitoria_3, monitoria_4, " +
      "m1_entered, m2_entered, m3_entered, m4_entered, " +
      "notes, created_at, updated_at" +
    ") " +
    "SELECT " +
      "id, user_id, reference_month, score, " +
  ...
  await connection.prepare("DROP TABLE quality_scores").run();
  await connection.prepare("ALTER TABLE quality_scores__new RENAME TO quality_scores").run();
  await connection.prepare("CREATE INDEX IF NOT EXISTS idx_quality_scores_user_month ON quality_scores(user_id, reference_month)").run();
  await connection.prepare("CREATE INDEX IF NOT EXISTS idx_quality_scores_reference_month ON quality_scores(reference_month)").run();
  await connection.prepare("CREATE UNIQUE INDEX IF NOT EXISTS idx_quality_scores_user_month_scope ON quality_scores(user_id, reference_month, quality_scope)").run();
}
